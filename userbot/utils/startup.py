    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await catub.tgbot.send_file(
                ...
                caption="**Your Premium Amm has been started successfully.**",
                buttons=[(Button.url("Owner", "https://t.me/Ammuyee"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "Premium Amm BotLog Group", catub, Config.TG_BOT_USERNAME, descript
        )
