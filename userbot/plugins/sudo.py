...
@catub.cat_cmd(
    ...
    info={
        "header": "To enable or disable sudo of your Premium Amm.",
...
async def chat_blacklist(event):
    "To enable or disable sudo of your Premium Amm."
...
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "__There are no sudo users for your Premium Amm.__"
        )
    result = "**The list of sudo users for your Premium Amm are :**\n\n"
...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    if input_str[0] == "-all":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your Premium Amm__.\n"
                )
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += f"`{cmd}` __There is no such command in your Premium Amm__.\n"
    ...
    result = f"__Successfully enabled __ `{len(loadcmds)}` __ for Premium Amm sudo.__\n"
    output = (
...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    result = f"__Successfully disabled __ `{count}` __ for Premium Amm sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
